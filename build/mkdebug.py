...
hook = """
  ...
  window.__dbg = {
    ...
    bots:()=>racers.filter(r=>!r.isPlayer).map(r=>({n:r.name, x:Math.round(r.x), y:Math.round(r.y), vy:+r.vy.toFixed(2), tx:Math.round(r.targetX||0), thr:r.aiThrottle, falls:r.fallCount, hf:r.holeFalls||{}, falling:!!r.falling, esc:+(r.escapeT||0).toFixed(1), pw:r.pitWait?{xw:Math.round(r.pitWait.xWait), c:r.pitWait.committed, w:+r.pitWait.waited.toFixed(1), f:r.pitWait.falls}:null})),
